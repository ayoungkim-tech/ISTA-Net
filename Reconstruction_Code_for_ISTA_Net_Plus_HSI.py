...
# Each test image data contains 31 images per channel of 512x512 spatial size.
# The test data value should be 0-255.
Test_Img = './Test_Image_ICVL/03' # Change the folder name
ch = 31;


def imread_CS_py(imgName):
    block_size = 33
    Iorg = sio.loadmat(imgName)
    Iorg = Iorg['ch_image']
...
filepaths = glob.glob(Test_Img + '/*.mat')
...
[Iorg, row, col, Ipad, row_new, col_new] = imread_CS_py(filepaths[1])

Iorg_hs = np.zeros([row, col, ch])
X_rec_hs = np.zeros([row, col, ch])
rec_PSNR_hs = np.zeros([1, ch])
...
for img_no in range(ImgNum):
    ...
    Iorg_hs[:,:,img_no] = Iorg
    X_rec_hs[:,:,img_no] = X_rec * 255
    rec_PSNR_hs[:,img_no] = rec_PSNR
    
rec_PSNR_hs = np.mean(rec_PSNR_hs)
    
imgName  = imgName[-5:]
print("Run time for %s is %.4f, PSNR is %.2f, loss sym is %.4f" % (imgName, (end - start), rec_PSNR_hs, cost_sym_value))

#x_im_rec = Image.fromarray(np.clip(X_rec_hs * 255, 0, 255).astype(np.uint8))

out_dict = {'rec_image': X_rec_hs,
            'gt_image': Iorg_hs}
out_filename = '%s/reconstruction.mat' % (Test_Img)
sio.savemat(out_filename, out_dict)


output_data = "Avg PSNR is %.2f dB, cpkt NO. is %d \n" % (rec_PSNR_hs, cpkt_model_number)
print(output_data)
